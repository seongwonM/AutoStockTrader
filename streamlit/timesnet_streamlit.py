...
def get_model_prediction(stock_code, current_hour_key):
    # current_hour_key 이전 36개 데이터 가져오기
    cursor.execute('SELECT * FROM price_info WHERE stock_code = ? AND time_key < ? ORDER BY time_key DESC LIMIT ?', (stock_code, current_hour_key, 28))
