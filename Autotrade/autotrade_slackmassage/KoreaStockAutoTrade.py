...
def send_message(msg):
    ...
    message = {
        "text": f"[{now.strftime('%Y-%m-%d %H:%M:%S')}] {str(msg)}"
    }
    client.chat_postMessage(channel=channel, text=message['text'])
...
def get_target_price(code="005930"):
    ...
    "fid_input_iscd":code,
    "fid_org_adj_prc":"1",
    "fid_period_div_code":"D"  # 시간단위 조회는 불가능..ㅠ 일, 주, 달 가능
    }
    res = requests.get(URL, headers=headers, params=params)
    ###### 여기서 일단위 대신 시간단위 시가,고가,저가 #####
...
def get_stock_balance():
    ...
    time.sleep(0.1)
    send_message(f"총 평가 금액: {int(evaluation[0]['tot_evlu_amt'])  + int(evaluation[0]['evlu_pfls_smtl_amt'])} 원")
    time.sleep(0.1)
    send_message(f"=================")
    return stock_dict
